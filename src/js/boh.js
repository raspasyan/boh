...
document.addEventListener("DOMContentLoaded", function () {
    ...
    cvs.addEventListener("mousemove", function (e) {
        mousePos = getMousePos(cvs, e);
        if (controller.mouse && (!lastMousePos || mousePos[0] != lastMousePos[0] || mousePos[1] != lastMousePos[1])) click(mousePos);
    });
    ...
    function draw() {
        ...
        // Draw sprites
        let sprites = objects.filter(function (obj) { return obj.type == "sprite"; });
        if (sprites.length) drawSprites(sprites);
    ...
    function drawProjectiles(projectiles, creatures) {
    ...
    function click(mousePos) {
        ...
        if (mousePos[0] <= view.width && mousePos[1] <= view.height) {
            ...
            editorSpriteOffsets = [Math.floor((mousePos[0] - SPRITE_SIZE / 2) / SPRITE_SIZE), Math.floor((mousePos[1] - SPRITE_SIZE / 2) / SPRITE_SIZE)];
        }
    }
